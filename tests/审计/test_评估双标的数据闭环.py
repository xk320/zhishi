...
class 双标数据闭环测试(unittest.TestCase):
    ...
    def test_BTC与ETH独立成员和状态守恒(self):
        with tempfile.TemporaryDirectory() as 目录:
            ...
            self.assertEqual(摘要["总成员数"], 630)
            self.assertEqual(摘要["最终状态计数"]["BTC"]["不可用"], 7)
            self.assertEqual(摘要["最终状态计数"]["ETH"]["不可用"], 7)
            成员路径 = Path(目录) / "批次" / "loop-test" / "闭环成员.csv"
            with 成员路径.open(encoding="utf-8", newline="") as 文件:
                行 = list(csv.DictReader(文件))
            self.assertEqual([行[0]["标的"], 行[314]["标的"], 行[315]["标的"]], ["BTC", "BTC", "ETH"])
            self.assertEqual(sum(记录["标的"] == "BTC" for 记录 in 行), 315)
            self.assertEqual(sum(记录["标的"] == "ETH" for 记录 in 行), 315)

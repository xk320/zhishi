...
class Stage1TimeQualityAuditTests(unittest.TestCase):
    ...
    def test_编译扫描器与Python合同输出一致(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            resources = {}
            scanner = self.auditor.compile_scanner(
                SCANNER_SOURCE, root, "/usr/bin/clang", process_resources=resources
            )
            name = "BTCUSDT-trades-2025-01-01"
            ...
            self.assertEqual("已证明", result["status"])
            self.assertEqual(2, result["row_count"])
            self.assertGreater(resources["compiler_max_rss_bytes"], 0)
            self.assertGreater(resources["unzip_max_rss_bytes"], 0)
            self.assertGreater(resources["scanner_max_rss_bytes"], 0)
            path.unlink()
            path = self.make_zip(root, name, [
    ...
    def test_最终真实批次计数指纹与门禁守恒(self):
        ...
        self.assertEqual(self.auditor.sha256_file(SCANNER_SOURCE), summary["scanner_source_sha256"])
        resources = summary["process_group_resource_facts"]
        self.assertEqual("zhishi-process-group-rusage/v1", resources["measurement_protocol"])
        self.assertEqual(1, resources["members_parallelism"])
        self.assertEqual(
            resources["compiler_max_rss_bytes"]
            + resources["unzip_max_rss_bytes"]
            + resources["scanner_max_rss_bytes"],
            resources["children_conservative_sum_max_rss_bytes"],
        )
        self.assertEqual(
            resources["controller_max_rss_bytes"]
            + resources["children_conservative_sum_max_rss_bytes"],
            resources["conservative_process_group_max_rss_bytes"],
        )
        self.assertLessEqual(resources["conservative_process_group_max_rss_bytes"], 512 * 1024 * 1024)
        actual = {

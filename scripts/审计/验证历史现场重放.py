...
def build_formal_coverage(
    ...
    if replay_evidence is not None and (
        type(replay_evidence) is not dict or len(replay_evidence) != 0
    ):
        raise ValueError("source_provenance_unverified")
...
def render_report(rows: Sequence[Mapping[str, str]], metadata: Mapping[str, str]) -> str:
    ...
    lines.extend([
        ...
        "- v1正式构建器的证据参数只允许None或精确内建空字典；其他类型、字典子类和任何非空内建字典均以`source_provenance_unverified`失败安全。",

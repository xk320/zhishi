...
EXPECTED_CONFIG_SHA256 = "d1bc5118ee091f888502f6e6509c94007a603ebea026c12631637489e217165d"
...
def compile_scanner(
    ...
    if completed.returncode != 0 or len(completed.stderr) > 65536 or not target.is_file() or target.is_symlink() or not os.access(target, os.X_OK):
...
def scan_member(
    ...
    awk_process: subprocess.Popen[bytes] | None = None
    try:
        ...
        stdout, awk_stderr = awk_process.communicate(timeout=int(limits["member_seconds"]))
        unzip_stderr = unzip_process.stderr.read(int(limits["scanner_stderr_bytes"]) + 1) if unzip_process.stderr else b""
        if unzip_process.stderr is not None:
            unzip_process.stderr.close()
        unzip_code = unzip_process.wait(timeout=10)
    except (OSError, subprocess.TimeoutExpired):
        for process in (awk_process, unzip_process):
    ...
    with tempfile.TemporaryDirectory(prefix="zhishi-task094-scanner-") as directory:
        ...
        for index, member in enumerate(members, start=1):
            ...
            try:
                ...
                result = scan_member(path, member, group, tools, limits)
...
def run(config_path: Path, repo_root: Path, output_root: Path, batch_id: str) -> Path:
    ...
    results, scanner_facts = scan_all_members(members, group_map, config, repo_root, output_root, source_root, started)
